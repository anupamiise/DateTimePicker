...
@interface DateTimePickerViewController ()<UIPickerViewDataSource, UIPickerViewDelegate>
...
- (NSString *)confirmButtonTitle {
    if (!_confirmButtonTitle) {
        _confirmButtonTitle = NSLocalizedString(@"Set date", @"DatePicker confirm button title");
    }
...
- (NSString *)backButtonTitle {
    if (!_backButtonTitle) {
        _backButtonTitle = NSLocalizedString(@"Back", @"DatePicker back button");
    }
...
- (IBAction)confirmDate:(id)sender {
    //TODO: Set date
    if ([self.delegate respondsToSelector:@selector(DatePickerPickedDate:)]) {
        [self.delegate DatePickerPickedDate:[self dateWithSelectedTime]];
    }
    
    if ([self.delegate respondsToSelector:@selector(DatePickerWillDismissWithQuitMethod:)]) {
        [self.delegate DatePickerWillDismissWithQuitMethod:QuitWithResult];
    }
    void (^success)(void) = nil;
    if ([self.delegate respondsToSelector:@selector(DatePickerDidDismissWithQuitMethod:)]) {
        success = ^{
            [self.delegate DatePickerDidDismissWithQuitMethod:QuitWithResult];
        };
    }
...
- (IBAction)quitPicking:(id)sender {
    if ([self.delegate respondsToSelector:@selector(DatePickerWillDismissWithQuitMethod:)]) {
        [self.delegate DatePickerWillDismissWithQuitMethod:QuitWithBackButton];
    }
    void (^success)(void) = nil;
    if ([self.delegate respondsToSelector:@selector(DatePickerDidDismissWithQuitMethod:)]) {
        success = ^{
            [self.delegate DatePickerDidDismissWithQuitMethod:QuitWithBackButton];
        };
    }
    [self dismissViewControllerAnimated:YES completion:success];
}
...
- (void)cancelTapGesture:(UITapGestureRecognizer *)sender {
    if (self.shouldDismissOnCancelTouch) {
        if ([self.delegate respondsToSelector:@selector(DatePickerWillDismissWithQuitMethod:)]) {
            [self.delegate DatePickerWillDismissWithQuitMethod:QuitWithCancel];
        }
        void (^success)(void) = nil;
        if ([self.delegate respondsToSelector:@selector(DatePickerDidDismissWithQuitMethod:)]) {
            success = ^{
                [self.delegate DatePickerDidDismissWithQuitMethod:QuitWithCancel];
            };
        }
        [self dismissViewControllerAnimated:YES completion:success];
    }
